...
export const fetchActivites = createAsyncThunk<Activity[], ActivityQuery>(
    ...
    async ({ destination, date, adults, children }) => {
        const response = await fetchActivityData({ destination, date, adults, children });
...
export const { setDestination, setDate, setAdults, setChildren, clearDestination, resetActivites } = activitySlice.actions;
