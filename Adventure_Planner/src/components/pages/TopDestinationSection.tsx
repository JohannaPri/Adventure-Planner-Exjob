...
const TopDestinationSection = () => {
  ...
  return (
    <section
      id="top-destinations"
      className="relative flex flex-col items-center justify-center w-full h-screen px-6 my-20 lg:px-24 md:px-20"
    >
      <main className="flex flex-col items-center justify-center w-full gap-3 pt-0">
        ...
        <div className="flex items-center justify-end w-full gap-5 px-3 mt-12 md:px-6">
          <Button
            onClick={previous}
            className="p-2 border-none rounded-full outline-none cursor-pointer bg-cloudGray text-slateGray hover:bg-cloudGray2 hover:text-black"
            type="button"
          >
          ...
          <Slider ref={(slider) => (sliderRef.current = slider)} {...settings}>
            {TopDestinationTexts.cards.map((card, index) => (
              <div key={index} className="px-3 md:px-6">
                <Card
                  cardClass="overflow-hidden shadow-md rounded-lg cursor-default group"
