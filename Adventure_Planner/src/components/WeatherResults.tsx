...
const WeatherResults: React.FC = () => {
  const { data: weatherData, status } = useSelector(
  ...
  if (hasSearched && (!weatherData || weatherData.length === 0)) {
    return (
      <div className="text-center transition duration-300 max-w-[300px] max-h-[317px] p-4 mx-auto text-black shadow-2xl shadow-black rounded-lg">
        <div className="absolute inset-0 bg-gradient-to-r to-orange-50 from-orange-200 opacity-85 rounded-lg z-0"></div>
        <div className="relative z-5">
          <h2 className="text-xl md:text-2xl font-semibold text-slateGray mb-4">
            Oops, no weather updates! 🌧️🤔
  ...
  return (
    <>
      {weatherData?.map((weather, index) => (
        <div
          key={index}
          className="text-center transition duration-300 min-w-[300px] h-[317px] min-h-[317px] max-h-[317px] p-4 mx-auto text-black shadow-2xl shadow-black rounded-lg"
        >
          <div className="absolute inset-0 bg-gradient-to-r to-orange-50 from-orange-200 opacity-85 rounded-lg z-0"></div>
          <div className="relative z-5">
            <div className="font-semibold text-xl text-shadow">
              {weather.name}, {weather.sys.country}
            </div>
            <div className="text-sm text-shadow">{dateBuilder()}</div>
            <div className="bg-white w-fit p-4 pl-5 pr-5 m-4 mx-auto rounded-xl font-bold text-3xl bg-opacity-70 shadow-md shadow-gray-800 text-shadow tracking-wide">
            ...
            <div className="text-xs font-medium tracking-wide uppercase p-1">
