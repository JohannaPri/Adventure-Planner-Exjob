...
/**
 * Component that fetches a specific subfolder's details and displays its title.
 *
...
const GetNewSubFolder: React.FC<GetSubFolderProps> = ({
  userId,
  parentFolderId,
  subFolderId,
}) => {
  ...
  useEffect(() => {
    const fetchSubFolder = async () => {
      ...
      try {
        const subFolder = await getSubFolderById(
          userId,
          parentFolderId,
          subFolderId
        );
    ...
    <div className="w-full">
      ...
      <div className="text-center text-sm sm:text-sm lg:text-lg md:text-lg max-w-md mx-auto text-gray-800">
        <p className="mb-2">
          ...
          put the finishing touches here. ✨
        </p>
        <p>
          If you go back a step, you can save flights, accommodations, and
          activities in the existing folders or create new Wanderlists. Give
          them a title that sparks excitement and matches the adventure you're
          planning.
        </p>
