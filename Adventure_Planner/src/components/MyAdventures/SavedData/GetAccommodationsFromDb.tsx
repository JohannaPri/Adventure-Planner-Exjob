...
const GetAccommodationsFromDb = ({ folderId }: { folderId: string }) => {
  ...
  const handleRemoveHotel = (
    ...
    .then(() => {
      setSavedHotels(prevHotels => prevHotels.filter(savedHotel => savedHotel.id !== hotel.id));
    }).catch(error => {
  ...
  return (
    <div className="space-y-2 w-full max-h-[500px] scroll-smooth mt-10 overflow-y-auto no-scrollbar last:mb-5 pb-10">
      {savedHotels.length > 0 ? (
        ...
        <p className="text-center max-w-md mx-auto text-gray-800">You haven’t saved any accommodations yet. Why not head over to Search and find the perfect place to stay for your next adventure? 🏨🌍</p>
      )}
    </div>
  );
};
