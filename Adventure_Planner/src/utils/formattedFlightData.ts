...
export function formatFlightData(data: FlightData[]): FormattedFlightData[] {
  return data.map((flight) => {
    ...
    const outboundCarrier =
      outbound?.airplane?.carrierName || "Unknown Carrier";
    ...
    const outboundDepartureTime = outbound?.departureTime
      ? new Date(outbound.departureTime).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        })
      : "N/A";
    const outboundArrivalTime = outbound?.arrivalTime
      ? new Date(outbound.arrivalTime).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        })
      : "N/A";
    ...
    const outBoundStops =
      outbound?.numberOfStops === 0
        ? "direct"
        : `${outbound?.numberOfStops} stops`;

    const returnFlight = flight.travel[1]?.segments[0];
    const returnCarrier =
      returnFlight?.airplane?.carrierName || "Unknown Carrier";
